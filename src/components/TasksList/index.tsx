...
function TasksList({id,content,status,tasks,setTasks}:props){  
    ...
    const handleClickTrash = (e:any)=>{
        const taskNew = tasks.filter((task)=> task.id != e.currentTarget.id)
            setTasks(taskNew)
        ...
        <div  className="shadow-lg flex  justify-between items-center mt-4  bg-grayToDoList500 rounded-lg border border-[#333333] p-4 gap-3 relative">
            ...
            <p className={` transition-all  break-all px-4 relative block flex-grow text-white ${status? ' line-through opaci   ty-50':''}`}>{content}</p>
            <BiTrash className="h-5 w-5  text-white" id={`${id}`} onClick={handleClickTrash}  ></BiTrash>
        </div>
            </>
        )
}
